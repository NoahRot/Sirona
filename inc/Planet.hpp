...
typedef std::string PlanetSubtype;
...
PlanetCategory string_to_planet_category(const std::string& str) {
    if (str == "Telluric")      return PlanetCategory::Telluric;
    if (str == "IceGiant")      return PlanetCategory::IceGiant;
    if (str == "GasGiant")      return PlanetCategory::GasGiant;
    if (str == "Exotic")        return PlanetCategory::Exotic;
    if (str == "Artificial")    return PlanetCategory::Artificial;
    
    // Fallback / error case
    throw std::string("WARNING::StellarObjectFactory: Unknown stellar category: " + str);
}

const char* planet_category_to_string(PlanetCategory category) {
    switch (category) {
        case PlanetCategory::Telluric:      return "Telluric";
        case PlanetCategory::IceGiant:      return "Ice Giant";
        case PlanetCategory::GasGiant:      return "Gas Giant";
        case PlanetCategory::Exotic:        return "Exotic";
        case PlanetCategory::Artificial:    return "Artificial";
...
enum class PlanetModifiers {
    ...
    // Rare
    // =========================
    AncientMegastructure,
    SleepingGodPriest,
    DreamRealm,
};
*/

struct PlanetSubtypeData {
...
public:
    ...
    PlanetObject generate_planet_object(AMB::Lehmer32& lehmer) {
        PlanetObject object;
        ...
        PlanetSubtypeData data = m_subtype_database[object.subtype];
    ...
    std::string info_category_str() const {
        ...
        for (const auto& cat : m_category_entries) {
            PlanetCategory category = cat.value;
            float probability = cat.weight;

            oss << "Category: " << std::setw(16) << planet_category_to_string(category) << ", probability: " << std::setw(4) << probability << "\n";
        }
    ...
    std::string info_subtype_str() const {
        ...
        for (const auto& data : m_subtype_database) {
            PlanetCategory category = data.second.category;
            PlanetSubtype subtype = data.second.subtype;
            float weight = data.second.weight;

            oss << "Category: " << std::setw(16) << planet_category_to_string(category) << ", subtype: " << std::setw(16) << subtype << ", weight: " << std::setw(4) << weight << "\n    Data (" << data.second.data.size() << "): \n";
...
private:
    std::vector<WeightedEntry<PlanetCategory>> m_category_entries;
    std::unordered_map<PlanetCategory, std::vector<WeightedEntry<PlanetSubtype>>> m_subtype_entries;
    std::unordered_map<PlanetSubtype, PlanetSubtypeData> m_subtype_database;

    const std::string CATEGORY_PATH = "res/data/planet_category.csv";
    const std::string SUBTYPE_PATH = "res/data/planet_subtype.csv";
    ...
    void load_category() {
        ...
        if (!file.is_open()) {
            m_logger.log(AMB::Fatal, "PlanetFactory: Can not open category file: " + CATEGORY_PATH);
            exit(EXIT_FAILURE);
        }
        ...
        while (std::getline(file, line)) {
            ...
            try {
                ...
                std::string error_message = "PlanetObjectFactory: Failed to parse probability for " + category_str + ": " + weight_str;
    ...
    void load_subtype() {
        ...
        if (!file.is_open()) {
            m_logger.log(AMB::Fatal, "PlanetFactory: Can not open subtype file: " + SUBTYPE_PATH);
            exit(EXIT_FAILURE);
        }
        ...
        while (std::getline(file, line)) {
            ...
            if (first_line) {
                ...
                if (header[0] != "Subtype" or header[1] != "Category" or header[2] != "Weight") {
                    std::string error_message = "PlanetFactory: Subtype file do not have a correct layout. Column one must be 'Subtype', column two must be 'Category', column two must be 'Weight'";
            ...
            PlanetSubtype subtype = subtype_str;
            ...
            try {
                ...
                std::string error_message = "PlanetFactory: Can not read weight of subtype: " + subtype_str;
            ...
            while(std::getline(ss, element_str, ',')) {
                ...
                try {
                    ...
                    std::string error_message = "PlanetFactory: Can not read subtype parameter: " + header[counter] + " = " + element_str;
                    m_logger.log(AMB::Error, error_message);
                    error_subtype = true;
                }
        ...
        for (auto [subtype, data] : m_subtype_database) {
            for (uint32_t i(3) ; i < header.size() ; ++i) {
                ...
                if (data.data.find(h) == data.data.end()) {
                    std::string error_message = "PlanetFactory: Element is missing in at least on subtype. Subtype=" + subtype + ", element=" + h;
                    m_logger.log(AMB::Fatal, error_message);
                    exit(EXIT_FAILURE);
                }
            }
